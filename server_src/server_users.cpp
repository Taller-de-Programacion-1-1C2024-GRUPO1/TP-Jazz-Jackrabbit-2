...
User::User(std::shared_ptr<ContainerProtocol> conteiner_protocol, MonitorMatches& monitor_matches, bool* playing) : status(ACTIVE), container_protocol(container_protocol), monitor_matches(monitor_matches), playing(playing) {}
    ...
    try {
        while (status == ACTIVE) {
            ...
            if (new_match->get_commandType() == NEW_MATCH) {
                create_new_match(new_match->get_number_players(), new_match->get_match_name(), new_match->get_map_name());
...
void User::create_new_match(int number_players, std::string match_name, std::string map_name) {
    /*
    ---- Primero, se obtiene el mapa que se quiere jugar y se crea una lista de conejos que van a jugar en el mapa. ----
    ...
    std::shared_ptr<GameMap> game_map = std::make_shared<GameMap>(GameMap(numberPlayers, map_name, map));
    std::shared_ptr<MatchInfo> new_match = std::make_shared<MatchInfo>(match_name, game_map, playing, protocols_queue);
...
void User::join_match(std::string match_name) {
...
std::unordered_map<int, Player> User::make_players_map(std::vector<Player>& player) {
