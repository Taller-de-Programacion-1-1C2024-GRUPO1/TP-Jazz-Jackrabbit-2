...
void PhysicalMap::check_colision_with_map(int pos_x, int pos_y, int width, int height,
    ...
    if (((phisicaMap[x0][y2]) + (phisicaMap[x1][y2]) + (phisicaMap[x2][y2])) > 4) {
        // printf("on floor");
        character->is_on_floor();
    }
    if (((phisicaMap[x0][y0]) + (phisicaMap[x1][y0]) + (phisicaMap[x2][y0])) > 4) {
        // printf("on roof");
        character->is_on_roof();
    }
    if (((phisicaMap[x0][y0]) + (phisicaMap[x0][y1]) + (phisicaMap[x0][y2])) > 4)  {
        // printf("on left wall");
        character->is_on_left_wall();
    }
    if (((phisicaMap[x2][y0]) + (phisicaMap[x2][y1]) + (phisicaMap[x2][y2])) > 4) {
        // printf("on right wall");
        character->is_on_right_wall();
    }


    if ((phisicaMap[x0][y2]) == 1 || (phisicaMap[x0][y1]) == 1){
        character->is_on_left_slope();
    } else if ((phisicaMap[x2][y2]) == 2 || (phisicaMap[x2][y1]) == 2){
        character->is_on_right_slope();
    }

    if (phisicaMap[x2][y2]==1 || phisicaMap[x2][y1]==1){
        character->is_on_right_wall();
    } else if (phisicaMap[x0][y2]==2 || phisicaMap[x0][y1]==2){
        character->is_on_left_wall();
    }
}
